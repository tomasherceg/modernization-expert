...
namespace ModernizationDemo.EfCoreEdmxTests
{
    ...
    {
        private const string connectionString = "metadata=res://*/Model.DotNetCollegeContext.csdl|res://*/Model.DotNetCollegeContext.ssdl|res://*/Model.DotNetCollegeContext.msl;provider=System.Data.SqlClient;provider connection string=\"data source=.\\SQLEXPRESS;initial catalog=EFTests;integrated security=True;trustservercertificate=True;MultipleActiveResultSets=True;App=EntityFramework\"";
        ...
        public void InsertUpdateDelete()
        {
            ...
            Assert.AreEqual(1, context.Database.SqlQuery<int>("SELECT COUNT(*) as Value FROM Courses WHERE Price = 12345").First());
            Assert.AreEqual(2, context.Database.SqlQuery<int>("SELECT COUNT(*) as Value FROM CourseDates WHERE CourseId = 6").First());

            // ensure the course was updated
            Assert.AreEqual(1, context.Database.SqlQuery<int>("SELECT COUNT(*) as Value FROM Courses WHERE Id = 1 AND IsApproved = 1 AND Price = 10000").First());

            // ensure the course was deleted
            Assert.AreEqual(0, context.Database.SqlQuery<int>("SELECT COUNT(*) as Value FROM Courses WHERE Id = 2").First());
        }
