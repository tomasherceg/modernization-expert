...
public class MembershipMigrator
{
    ...
    public MembershipMigrator(UserManager<AppUser> userManager, RoleManager<AppRole> roleManager, AppDbContext dbContext)
    {
    ...
    private async Task MigrateUsers(SqlConnection connection)
    {
        var users = ReadDataRows(connection, command =>
        {
            command.CommandText =
                """
                SELECT m.*, u.UserName, u.LoweredUserName, u.MobileAlias, u.IsAnonymous, u.LastActivityDate
        ...
        foreach (var user in users)
        {
            ...
            // We have no MobileAlias in the new system
            ...
            var passwordHasher = (MembershipAwarePasswordHasher)userManager.PasswordHasher;
        ...
        var roles = ReadDataRows(connection, command =>
        {
            command.CommandText =
                ...
                SELECT r.*
                  ...
                  JOIN dbo.aspnet_Applications a ON a.ApplicationId = r.ApplicationId
                  WHERE a.ApplicationName = @applicationName
                """;
            command.Parameters.AddWithValue("@applicationName", ApplicationName);
        });
        ...
        var usersRoles = ReadDataRows(connection, command =>
        {
            command.CommandText =
                ...
                SELECT ur.UserId, r.RoleName
                  ...
                  JOIN dbo.aspnet_Roles r ON r.RoleId = ur.RoleId
                  JOIN dbo.aspnet_Applications a ON a.ApplicationId = r.ApplicationId
                  WHERE a.ApplicationName = @applicationName
                """;
            command.Parameters.AddWithValue("@applicationName", ApplicationName);
    ...
    private async Task MigrateProfiles(SqlConnection connection)
    {
        ...
        foreach (var profile in profiles)
        {
            ...
            var properties = ProfileParser.ParseProperties(
                ...
                (string)profile["PropertyValuesString"], 
                (byte[])profile["PropertyValuesBinary"]);
            foreach (var property in properties)
            {
                switch (property.Name)
                {
                    ...
                    case "ImageUrls":
                        ...
                        break;
                    case "ShoppingCart":
                        user.ShoppingCart = ProfileParser.DeserializeShoppingCart(property.BinaryValue!);
                        break;
                    default:
